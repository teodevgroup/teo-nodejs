...
use crate::{headers::headers::Headers, object::{js_any_to_teo_value, value::teo_value_to_js_any_no_map}, request::Cookie};
...
impl Response {
    ...
    pub fn cookies(&self) -> Vec<Cookie> {
        self.original.cookies().into_iter().map(|c| Cookie { inner: c }).collect()
    }
}
...
impl FromNapiValue for Response {
    unsafe fn from_napi_value(env: napi::sys::napi_env, napi_val: napi::sys::napi_value) -> Result<Self> {
        ...
        Ok(Response { original: response.original.clone() })
    }
}
